...
class Database:
    ...
    def check_unique_mol(self, molname):
        cursor = self.conn.cursor()
        cursor.execute("SELECT * FROM Molecules WHERE NAME = ?", (molname,))
        #if that returns true, then it is not unique, and send back false
        rows = cursor.fetchall()
        cursor.close()
        print(bool(rows))
        return bool(rows)
    
    def check_unique_element(self, element_code):
        cursor = self.conn.cursor()
    ...
    def get_Molecules(self):
        cursor = self.conn.cursor()
        cursor.execute("""
            SELECT m.NAME, COUNT(DISTINCT a.ATOM_ID), COUNT(DISTINCT b.BOND_ID)
            FROM Molecules m 
    ...
    def clear_molecule(self, molecule_name):
        cursor = self.conn.cursor()
        cursor.execute("""
        ...
        cursor.execute("DELETE FROM MoleculeAtom WHERE MOLECULE_ID = ?", (molecule_id,))
        cursor.execute("DELETE FROM MoleculeBond WHERE MOLECULE_ID = ?", (molecule_id,))
        cursor.execute("DELETE FROM Atoms WHERE ATOM_ID IN (SELECT ATOM_ID FROM MoleculeAtom WHERE MOLECULE_ID = ?)", (molecule_id,))
        cursor.execute("DELETE FROM Bonds WHERE BOND_ID IN (SELECT BOND_ID FROM MoleculeBond WHERE MOLECULE_ID = ?)", (molecule_id,))
        self.conn.commit()
    ...
    def get_Elements (self):
        cursor = self.conn.cursor()
        cursor.execute("""
                       SELECT ELEMENT_NAME, ELEMENT_CODE, COLOUR1 FROM Elements
                       """)
        elements = cursor.fetchall()
        return elements
    
    def del_Elements (self, element):
        cursor = self.conn.cursor()
        cursor.execute("""
                       DELETE FROM Elements WHERE ELEMENT_NAME = ?
                       """, (element,))
        self.conn.commit()
